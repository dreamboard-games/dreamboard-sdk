...
- Hosts:
  ...
  - **PluginRuntime** (client side): `createPluginRuntimeAPI.ts`'s
    `console.error(\`[Plugin RuntimeAPI] ...\`)`path gains an optional`onDiagnostic`prop on`PluginRuntime`, defaulting to the current
...
- **Ordering with the executor**: deleting the global before the executor
  ...
  N keeps reading the global _and_ supports the sink; hosts migrate; SDK
