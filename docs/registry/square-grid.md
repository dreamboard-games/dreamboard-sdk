...
```sh
pnpm dlx shadcn@4.21.0 add @dreamboard/square-grid
```

Pure, composable square grid presentation. No SDK dependency.
...
`SquareGrid` takes a label, cellSize and cells with id/x/y plus optional fill/label. SVG children compose overlays; pass a viewBox suitable for the supplied coordinates.
