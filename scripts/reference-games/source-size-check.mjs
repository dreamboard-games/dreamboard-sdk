...
Read the Git index and enforce the structural reference-game source inventory.
Audit mode reports every violation but exits successfully.
`.trim();
}
...
function collectReceipt() {
  ...
  return {
    schemaVersion: 1,
    source: "git-index",
    invariants: { retainedLockCount: expectedLockCount },
    summary: {
...
function printHuman(receipt, audit) {
  ...
  process.stdout.write(
    `Reference-game source inventory ${mode}: ${receipt.summary.trackedPaths} paths, ` +
  ...
  if (receipt.violations.length === 0) {
    process.stdout.write("Source inventory policy passes.\n");
    return;
  }
  process.stdout.write(
    `${audit ? "Audit found" : "Source inventory policy failed with"} ${receipt.violations.length} violation(s):\n`,
  );
