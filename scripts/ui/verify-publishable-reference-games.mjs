...
async function validateDemoMetadata({
  ...
  if (!packageJson.scripts?.verify) {
    errors.push(`${gameId}: packageable demo must define a verify script`);
  }
  ...
  return {
    ...
    checks: {
      ...
      verify: packageJson.scripts?.verify ? "pending" : "missing",
      typecheck: "pending",
      test: "pending",
      testUi: "pending",
    },
...
async function runDemoChecks({ demo, tempRoot }) {
  ...
  run("pnpm", ["run", "verify"], { cwd: sandbox, stdio: "inherit" });
  demo.receipt.checks.verify = "passed";
  demo.receipt.checks.typecheck = "passed";
  demo.receipt.checks.test = "passed";
