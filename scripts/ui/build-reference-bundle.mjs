...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--sdk-tarball") {
      ...
      if (!value) throw new Error("--sdk-tarball requires a file path.");
      options.sdkTarball = path.resolve(root, value);
      index += 1;
      continue;
    }
...
async function main() {
  ...
  if (options.sdkTarball) {
    sdkTarballPath = path.join(sdkPackDir, path.basename(options.sdkTarball));
    await copyFile(options.sdkTarball, sdkTarballPath);
  } else {
    const sdkPack = run(
      "npm",
    ...
    sdkTarballPath = path.join(sdkPackDir, sdkTarballName);
  }
