...
async function checkDemoRegistryAbsence({ gameIds, errors }) {
  ...
  const forbiddenSourcePaths = new Map(
    gameIds.map((gameId) => [
      gameId,
      { value: `examples/reference-games/${gameId}`, unexpectedFiles: [] },
    ]),
  );
  for (const relative of repoFiles) {
    ...
    for (const entry of forbiddenSourcePaths.values()) {
      if (text.includes(entry.value)) {
        entry.unexpectedFiles.push(relative);
      }
    }
  }
  for (const [gameId, entry] of forbiddenSourcePaths) {
    if (entry.unexpectedFiles.length > 0) {
      errors.push(
        `${gameId}: reference game path appears outside reference/docs scope: ${entry.unexpectedFiles.join(", ")}`,
      );
    }
  }
}
...
async function main() {
  ...
  await checkDemoRegistryAbsence({ gameIds: selectedGameIds, errors });
