...
test("the daily gate excludes package and browser release proof", async () => {
  ...
  for (const releaseOnlyCommand of [
    ...
    assert.equal(
      manifest.scripts.check.includes(releaseOnlyCommand),
      false,
      `pnpm check must not invoke ${releaseOnlyCommand}`,
    );
  }
});
