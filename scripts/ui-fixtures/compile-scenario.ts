// @ts-nocheck -- This compiler executes authored, schema-validated modules with intentionally dynamic shapes. Its typed boundary is compileScenarioModule.
...
import { readJson, root } from "../ui/support.ts";
import { executeProtocolAuthority } from "./authority/protocol-authority.ts";
import { executeReducerAuthority } from "./authority/reducer-authority.ts";
import {
...
async function materializeWorkspaceScenario({
  ...
  if (metadata.schemaVersion !== 5) {
    throw new Error(
      `${scenario.id} reference-game UI compilation requires reference-game.json schemaVersion 5.`,
    );
  }
...
export async function compileScenarioModule({
  ...
  const sourceModulePath =
    metadata?.schemaVersion === 5 && existsSync(workspaceAppPath)
      ? workspaceAppPath
      : workspaceUiPath;
