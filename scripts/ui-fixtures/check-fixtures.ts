...
import { root } from "../ui/support.ts";
import {
...
function assertRenderModuleExternalized(
  relativePath: string,
  source: string,
): void {
  const forbidden = [
...
}: { readonly fixturesRoot?: string } = {}): Promise<{
...
function parseArgs(argv: readonly string[]): { fixturesRoot?: string } {
  const options: { fixturesRoot?: string } = {};
