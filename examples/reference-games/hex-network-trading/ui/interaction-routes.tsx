...
function ActionPanel({ title, detail, children }: ActionPanelProps) {
  return (
    <div className="grid gap-2 rounded-lg border-2 border-[#2d2d2d] bg-[#fdfbf7] p-2 text-sm">
      <div>
    ...
    <>
      ...
      <playLandmarkForm.State unavailable={null}>
        {(state) => {
          ...
          return (
            <ActionPanel
              title="Landmark"
              detail={`Selected ${selectedCardId}.`}
            >
      ...
      <playScoutForm.State unavailable={null}>
        {(state) => {
          ...
          if (!selectedCardId) return null;
          if (!selectedStormSpaceId) {
            return (
              <ActionPanel
                title="Play scout"
                detail="Choose a storm space on the board."
              >
