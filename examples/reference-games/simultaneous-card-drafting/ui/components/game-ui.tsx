import { PlayerRoster } from "../../shared/generated/ui-contract.ts";
import { literals, type PlayerId } from "@dreamboard/manifest-contract";
import {
...
function asPlayerId(value: string): PlayerId | null {
  return (literals.playerIds as readonly string[]).includes(value)
    ? (value as PlayerId)
    : null;
}
...
export function GameUI({
    ...
    phase === "drafting"
      ...
      : phase === "gameOver"
        ? winningPlayerIds.length > 0
          ? `Winner: ${winningPlayerIds
              .map((id) => {
                const typedPlayerId = asPlayerId(id);
                return typedPlayerId
                  ? (players.byId.get(typedPlayerId)?.name ?? typedPlayerId)
                  : id;
              })
              .join(", ")}`
          : "Final scores are in."
  ...
  return (
    <div className="min-h-screen bg-[#fff7ed] pb-[260px] text-[#2d2d2d] sm:pb-[280px] lg:pb-6">
      <header className="sticky top-0 z-30 border-b-2 border-[#2d2d2d] bg-[#fff7ed]/95 backdrop-blur-sm">
        <div className="mx-auto flex max-w-[1180px] flex-wrap items-center justify-between gap-3 px-4 py-3">
          ...
          <PlayerRoster.Root
            order="self-first"
            score={(playerId) => {
              const typedPlayerId = asPlayerId(playerId);
              return typedPlayerId
                ? (view.totalScoreByPlayer[typedPlayerId] ?? 0)
                : 0;
            }}
            scoreLabel="PTS"
          >
        ...
        >
          <div className="mx-auto grid max-w-[1180px] gap-3 px-3 py-3 lg:grid-cols-[minmax(0,1fr)_280px]">
            <section className="min-w-0">
              ...
              <hand.Hand
                className="flex min-h-[108px] flex-nowrap items-end gap-1.5 overflow-x-auto px-2 pb-2 sm:gap-2"
                  ...
                  {(card) =>
                    card.hidden ? (
                      ...
                      <div className="shrink-0">
                        <hand.Card
                          card={card}
                          className="relative h-[88px] w-[64px] overflow-hidden rounded-lg border-0 bg-transparent p-0 transition-shadow enabled:cursor-pointer data-[selected=true]:ring-2 data-[selected=true]:ring-[#ff4d4d] data-[eligible=false]:opacity-40 disabled:cursor-not-allowed disabled:opacity-40"
