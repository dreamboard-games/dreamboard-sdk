...
const SDK_GENERATED_DIR = path.join(
  REPO_ROOT,
  "packages",
  "sdk",
  "src",
...
const SDK_INFRASTRUCTURE_CONTRACT_DIR = path.join(
  REPO_ROOT,
  "packages",
  "sdk",
  "src",
...
fs.mkdirSync(SDK_GENERATED_DIR, { recursive: true });
fs.mkdirSync(SDK_INFRASTRUCTURE_CONTRACT_DIR, { recursive: true });
...
fs.writeFileSync(
  path.join(SDK_INFRASTRUCTURE_CONTRACT_DIR, "bundle.ts"),
  renderBundleContract({
    wireImportPath: "../../generated/reducer-contract/wire",
    authoredReducerImport: "@dreamboard-games/sdk/reducer",
  }),
);
...
for (const fileName of ["wire.ts", "zod.ts", "builders.ts", "version.ts"]) {
  fs.copyFileSync(
    path.join(OUT_DIR, fileName),
    path.join(SDK_GENERATED_DIR, fileName),
  );
}
...
console.log(
  `✓ reducer-contract codegen: emitted wire.ts, zod.ts, builders.ts, version.ts, bundle.ts, and SDK ABI mirrors (${pkg.version})`,
);
