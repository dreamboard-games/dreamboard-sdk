...
try {
  ...
  await run(
    [
      ...
      "shadcn",
      "add",
      ...manifest.items.map((item) => `@dreamboard/${item.name}`),
      "--yes",
      "--cwd",
      project,
    ],
  ...
  for (const item of manifest.items)
    await readFile(
      path.join(
        project,
        "src/components/dreamboard",
        `${item.name}.${item.name === "tokens" ? "css" : "tsx"}`,
      ),
    );
  console.log(
    "Installed and typechecked all registry items in an SDK-free consumer.",
  );
} finally {
  server.close();
